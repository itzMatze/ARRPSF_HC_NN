...
{
    __requirePrelude(R"(
void glsl_wmma_128_32_32_fused_preloadedweights(uint64_t weights_pointer, uint32_t num_layers)
{
    // load the data from shared memory to the fragment
    ...
    for (uint32_t j = 0; j < num_layers; j++)
    {
        ...
        uint inputs_offset = 4 * 32 * 32;
        uint layer_offset = 32 * 32 * j;

        uint weights_offset = inputs_offset + layer_offset;

        coopMatLoadNV(weights_frag[0], glsl_half_shared_buffer, weights_offset, 32, false);
        coopMatLoadNV(weights_frag[1], glsl_half_shared_buffer, weights_offset + 512, 32, false);
        coopMatLoadNV(weights_frag[2], glsl_half_shared_buffer, weights_offset + 16, 32, false);
        coopMatLoadNV(weights_frag[3], glsl_half_shared_buffer, weights_offset + 528, 32, false);
...
{
    __requirePrelude(R"(void glsl_wmma_128_32_32_fused(uint64_t weights_pointer, uint32_t num_layers)
{
...
{
    __requirePrelude(R"(void glsl_wmma_128_32_32_weightsvram(uint64_t weights_pointer)
{
     // load the data from shared memory to the fragment
     fcoopmatNV<16, gl_ScopeSubgroup, 16, 16> act_frag[4];
     fcoopmatNV<16, gl_ScopeSubgroup, 16, 16> weights_frag[4];
